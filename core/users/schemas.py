...
class UserBaseSchema(BaseModel):
    email: EmailStr | None = Field(
        None, max_length=100, description="Email of the user"
    )


class UserResponseSchema(UserBaseSchema):
    username: str = Field(..., max_length=50, description="Username of the user")
    user_register_type: UserRegisterType = Field(
...
class UserCreateSchema(UserBaseSchema):
    ...
    password: str = Field(
        ..., min_length=8, max_length=255, description="Password of the user"
    )
    confirm_password: str = Field(
        ..., min_length=8, max_length=255, description="Password confirmation"
    )
